#!/usr/bin/env python3
"""
Local runner for the reviewer-requested CIFAR-10 capacity-control baseline:

    frozen BLIP-2 vision encoder + MLP head with approximately the same
    trainable parameter count as DREAM.

Expected local data layout:

    --data-dir can be any writable directory. torchvision will download
    CIFAR-10 there when --download is passed.

Example:

    python blip_training_c10_mlp.py \
...
DEFAULT_HIDDEN_DIM = 10666
BLIP2_FEATURE_DIM = 1408


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-dir", required=True, help="Path where CIFAR-10 is stored/downloaded")
    parser.add_argument("--output-dir", default="outputs/runs/c10_blip2_vision_same_params_mlp")
...
class BLIP2VisionSameParamsMLP(nn.Module):
    def __init__(
        ...
        feature_dim = getattr(self.blip.config, "hidden_size", BLIP2_FEATURE_DIM)
...
def main():
    ...
    run_info = {
        ...
        "validation_split": "CIFAR10 train=False",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_top1 > best_top1:
            best_top1 = val_top1
            checkpoint_path = checkpoint_dir / f"c10_blip2_vision_same_params_mlp_{timestamp}_epoch_{epoch}.pth"
    ...
    print(f"Best validation Top-1 on CIFAR-10 test set: {best_top1:.2f}%")
